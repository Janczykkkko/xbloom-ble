...
# Pour-pattern codes on the cloud side: center=1, circular=2, spiral=3.
# This package's own recipe patterns are spiral / ring / center; "ring" maps to
# the cloud's "circular".
PATTERN_CODES = {"center": 1, "circular": 2, "ring": 2, "spiral": 3}
...
def _pour_to_cloud(pour: "Pour", index: int) -> dict[str, Any]:
    ...
    * ``pattern`` → 1/2/3 (center/circular/spiral); this package's "ring" → 2.
    * ``agitation`` (a spiral bloom flag) → ``isEnableVibrationBefore`` (1/2).
    * booleans are encoded 1 = on, 2 = off (never true/false).
    ...
    return {
        ...
        "isEnableVibrationBefore": _cloud_bool(bool(pour.agitation)),
        "isEnableVibrationAfter": _cloud_bool(False),
    }
...
class XBloomCloud:
    ...
    def update_recipe(
        self, table_id: int | str, recipe: "Recipe | dict[str, Any]", **map_kwargs: Any
    ) -> dict[str, Any]:
    ...
    def delete_recipe(self, table_id: int | str) -> dict[str, Any]:
